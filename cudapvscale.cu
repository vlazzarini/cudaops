...
static int cudapvscaleset(CSOUND *csound, CUDAPVSCALE *p)
{
  ...
  csound->Message(csound, "cudapvscale running on device %s (capability %d.%d)\n", deviceProp.name,
  ...
  // cudaMemcpy(p->deviceInput,p->fin->frame.auxp,size,cudaMemcpyHostToDevice);   // PROBABLY REDUNDANT
...
static int cudapvscale(CSOUND *csound, CUDAPVSCALE *p)
{
    ...
    if (p->lastframe < p->fin->framecount) {
      ...
      if (keepform == 0) {
        ...
        freqScaleBasic<<<p->gridSize,p->blockSize>>>(p->deviceInput, p->deviceOutput, pscal, Nhalf);   // freq scaling
        fixPVandGain<<<p->gridSize,p->blockSize>>>(p->deviceOutput, g, framelength);   // apply gain to all amplitudes 
      }
        ...
        freqScaleFormant<<<p->gridSize,p->blockSize>>>(p->deviceInput, p->deviceOutput, p->deviceEnv, pscal, max, Nhalf);   // normalize spectral env and freq scale the input
        fixPVandGain<<<p->gridSize,p->blockSize>>>(p->deviceOutput, g, framelength);   // apply gain to all amplitudes 
      }
        ...
        freqScaleFormant<<<p->gridSize,p->blockSize>>>(p->deviceInput, p->deviceOutput, p->deviceSmoothTrueEnv, pscal, max, Nhalf);   // normalize spectral env and freq scale the input
        fixPVandGain<<<p->gridSize,p->blockSize>>>(p->deviceOutput, g, framelength);   // apply gain to all amplitudes 
      }
...
static OENTRY localops[] = {
  {"cudapvscale", sizeof(CUDAPVSCALE),0, 3, "f", "fxOPO", (SUBR) cudapvscaleset,
   (SUBR) cudapvscale}
};
