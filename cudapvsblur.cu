...
typedef struct _cudapvsblur{
  ...
  AUXCH delframes;     // data structure to hold the PV data of current and past
		       // input data to be averaged (sliding mode only)
...
static int cudapvsblurset(CSOUND *csound, CUDAPVSBLUR *p)
{
  ...
  csound->Message(csound, "cudapvsblur running on device %s (capability %d.%d)\n", deviceProp.name,
  ...
  // ONLY FOR SLIDING MODE...DELETE THIS...
  if (p->delframes.auxp == NULL || p->delframes.size < (N + 2) * sizeof(float) * CS_KSMPS * maxframes)
    csound->AuxAlloc(csound, (N + 2) * sizeof(float) * maxframes, &p->delframes);
...
static int cudapvsblur(CSOUND *csound, CUDAPVSBLUR *p)
{
  ...
  if (p->lastframe < p->fin->framecount) {
    ...
    cudaMemcpy(p->deviceMatrix+(countr*framelength),fin,size,cudaMemcpyHostToDevice);
  
    if (delayframes) {
      if ((first = countr - delayframes) < 0)
        first += maxframes;
      
      blurnaive<<<p->gridSize,p->blockSize>>>(p->deviceMatrix, p->deviceOutput, first, delayframes, countr, maxframes, framelength);
...
static OENTRY localops[] = {
  {"cudapvsblur", sizeof(CUDAPVSBLUR),0, 3, "f", "fki", (SUBR) cudapvsblurset, (SUBR) cudapvsblur, NULL}
};
