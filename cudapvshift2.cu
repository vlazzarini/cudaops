...
static int cudapvshift2set(CSOUND *csound, CUDAPVSHIFT2 *p)
{
  ...
  csound->Message(csound, "cudapvshift2 running on device %s (capability %d.%d)\n", deviceProp.name,
...
static int cudapvshift2(CSOUND *csound, CUDAPVSHIFT2 *p)
{
    ...
    if (p->lastframe < p->fin->framecount) {
      ...
      if (keepform == 0) {
        ...
        freqShiftBasic<<<p->gridSize,p->blockSize>>>(fin, fout, pshift, cshift, lowest, Nhalf);   // freq shifting
        fixPVandGain<<<p->gridSize,p->blockSize>>>(fin, fout, g, lowest, framelength);   // apply gain to all amplitudes 
      }
        ...
        freqShiftFormant<<<p->gridSize,p->blockSize>>>(fin, fout, p->deviceEnv, pshift, cshift, lowest, max, Nhalf);   // normalize spectral env and freq scale the input
        fixPVandGain<<<p->gridSize,p->blockSize>>>(fin, fout, g, lowest, framelength);   // apply gain to all amplitudes 
      }
        ...
        freqShiftFormant<<<p->gridSize,p->blockSize>>>(fin, fout, p->deviceSmoothTrueEnv, pshift, cshift, lowest, max, Nhalf);   // normalize spectral env and freq scale the input
        fixPVandGain<<<p->gridSize,p->blockSize>>>(fin, fout, g, lowest, framelength);   // apply gain to all amplitudes 
      }
...
static OENTRY localops[] = {
  {"cudapvshift2", sizeof(CUDAPVSHIFT2),0, 3, "f", "fxkOPO", (SUBR) cudapvshift2set,
   (SUBR) cudapvshift2}
};
